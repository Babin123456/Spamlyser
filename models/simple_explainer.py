...
SPAM_KEYWORDS: Dict[str, List[str]] = {
    "Urgency / Pressure": [
        "urgent", "immediately", "act now", "don't miss", "expires",
        "limited time", "last chance", "hurry", "today only", "deadline",
    ],
    "Financial / Prize": [
        "winner", "won", "prize", "cash", "lottery", "million", "pounds",
        "dollars", "inheritance", "claim", "reward", "money", "fortune",
    ],
    "Security / Account": [
        "account", "verify", "bank", "login", "password", "security",
        "confirm", "update your", "suspicious", "unauthorized", "blocked",
    ],
    "Marketing": [
        "free", "discount", "offer", "trial", "subscribe", "exclusive",
        "limited offer", "buy now", "click here", "sale", "deal",
    ],
    "Personal Info": [
        "ssn", "social security", "credit card", "bank account", "paypal",
    ...
    def visualize_explanation(
        ...
        if spam_features:
            ...
            visualization["summary"] = (
                f"Top spam signal: '{top}' "
                f"(weight: {spam_features[0]['weight']:.2f})"
            )

        return visualization

    def get_threat_explanation(
        self, text: str, threat_type: Optional[str] = None
    ) -> Dict[str, Any]:
